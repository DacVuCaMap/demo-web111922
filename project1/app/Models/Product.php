...
class Product extends Model
{
    ...
    //get new products in homepage
    public function getnewpro(){
        $data=DB::select('SELECT pi.pro_id,pi.img_first,pr.pro_name,ca.name,pd.brand,pd.diment,pd.size from product pr
        inner join category ca on ca.id=pr.cat_id
        inner join proimage pi on pr.id=pi.pro_id
        inner join prodesc pd on pd.pro_id=pr.id
        order by date(pr.create_at) desc limit 4');
        return $data;

    }
    //categories
    public function afterFilter($brand,$cat){
        
        $data = DB::select("SELECT * FROM product pr INNER JOIN proimage pm on pr.id=pm.pro_id inner join prodesc pd on pd.pro_id=pr.id where pr.cat_id like '%{$cat}%' and pd.brand like '%{$brand}%'");
        return $data;
    }
}
